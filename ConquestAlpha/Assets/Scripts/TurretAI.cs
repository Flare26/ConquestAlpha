...
public class TurretAI : MonoBehaviour
{
    ...
    public Team enemyTeam; // Enemy tag can change relative to the team this turret belongs to. For now, just using NPCTank for testing.
    ...
    private void OnEnable()
    {
        ...
        InvokeRepeating("ConfirmAllegiance", 0f, 2f); // This is called when the turret is "Built"
    ...
    void UpdateTarget()
    {
        ...
        {
            if (enemy.GetComponent<NPCAI>().m_team == m_Team)
                return;
        ...
        if ( nearestEnemy != null && shortestDistance <= range && !Physics.Raycast(turretHead.transform.position, nearestEnemy.transform.position, 100))
        {
            //if nearest enemy is not null, and it's within our range, and the raycast does not hit a collider (false)
            m_Target = nearestEnemy.transform; // store the transform of our nearest target.
        } else
        {
    ...
    void Shoot()
    {
        ...
        if (bulletCS != null)
        {
            bulletCS.SetTargetTransform(this.m_Target);
        }
    }
    ...
    void Update()
    {
        ...
        Vector3 dir = m_Target.position - turretHead.transform.position; // get the difference between points
